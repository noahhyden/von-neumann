...
// ── papers: longer-form write-ups typeset in LaTeX (IEEE template) ───────────────
// The list (frontend/src/papers-index.ts) is generated by papers/scripts/gen-index.mjs
// from each paper's paper.json, and every cited id is validated against the same SOURCES
// bibliography that powers the site's <Cite> markers - a paper and a live surface point
// at the identical reference. The typeset PDF is compiled from LaTeX in CI and published
// under /papers/<slug>.pdf beside this app.
    ...
    <div>
      <section class="hero">
        <div class="wrap">
          ...
          <h1>Working papers, built from the same sourced models.</h1>
          <p class="lede">
            Longer-form write-ups that develop a single result in depth. Each is a LaTeX paper typeset with the IEEE template, and its bibliography is generated from the very same source list that powers the citations across this site - so a claim in a paper and a claim on a live surface point at the identical reference. The typeset PDF is linked from each entry.
          </p>
          <p class="note">
            {() => `${PAPERS.length} paper${PAPERS.length === 1 ? "" : "s"}. Working drafts; each PDF is compiled from LaTeX in CI and published beside this site.`}
          </p>
        </div>
      </section>

      <Show when={() => PAPERS.length > 0} fallback={(
        <section><div class="wrap"><p class="note">No papers published yet.</p></div></section>
      )}>
        <For each={() => PAPERS}>
          {(p: PaperMeta) => (
            <section>
                ...
                <For each={() => p.cites}>
                  {(id: string) => {
                    ...
                    return s ? (
                      <div class="src">
                        <div class="src-top">
                          <span class="src-num">[{sourceNumber(id)}]</span>
                          <span class="src-cite"><b>{s.authors}</b> ({s.year}). {s.title}. <i>{s.venue}</i>.</span>
                        </div>
                        {s.url ? <a class="src-link" href={s.url} target="_blank" rel="noopener noreferrer">{s.url}</a> : null}
                      </div>
                    ) : null;
                  }}
                </For>
      ...
      <footer>
        <div class="wrap">
          <p>
            Every reference above is drawn from the project <a href="#/sources" onClick={(e: Event) => { e.preventDefault(); mount("sources"); }}>bibliography</a>, so paper and site never drift. Back to the <a href="#/" onClick={(e: Event) => { e.preventDefault(); mount("overview"); }}>overview</a>.
          </p>
        </div>
      </footer>
    </div>
...
function Nav(props: { surface: Surface }) {
  return (
    <div class="wrap" style="padding-top:18px;display:flex;gap:8px;align-items:center;flex-wrap:wrap">
      ...
      <button class={`act ${props.surface === "papers" ? "primary" : "ghost"}`} onClick={() => mount("papers")}>Papers</button>
...
const VALID_SURFACES: Surface[] = [
  "overview", "mission", "fleet", "swarm", "spine", "wall", "probe", "launch", "power", "papers", "sources",
];
const SURFACE_TITLE: Record<Surface, string> = {
  ...
  papers: "Papers - The Arithmetic of Self-Replication",
...
function mount(surface: Surface, scenarioKey = "lunar", updateUrl = true) {
  ...
  } else if (surface === "papers") {
    disposeRender = render(() => (<div><Nav surface="papers" /><PapersSurface /></div>), appEl);
