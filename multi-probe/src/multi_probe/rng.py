...
``swarm/rng.py`` and ``frontend/scripts/gen-diff.mjs``; the JS parity is now
pinned by ``core/tests/test_rng.py`` against a committed fixture, so there is
one Python source of truth for the mulberry32. Reliability uses a distinct
64-bit generator (splitmix64) for its own reasons - see
``reliability/src/reliability/rng.py`` - sharing the ``(value, new_state)``
threading contract but not the algorithm. Existing call sites (``from
multi_probe.rng import next_float, seed_state``, etc.) keep working unchanged.
"""
