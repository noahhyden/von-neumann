...
| [`swarm`](swarm/) | 🚧 slice 1 + live surface | **How fast a probe fills the galaxy.** A deterministic, seeded settlement front: probes spread star-to-star through a field, settling and re-launching, filling the reachable galaxy outward from one homeworld (the exploration-timescale question of Nicholson & Forgan 2013). Slice 1 is the pure algorithm core, with a live canvas "Swarm" surface (play/scrub the fill). Slingshots, a 200k-star SoA + WebGL engine, and the novel light-speed-limited-coordination extension are later slices. |
